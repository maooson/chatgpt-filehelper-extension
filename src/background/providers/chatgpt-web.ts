...
const KEY_ACCESS_TOKEN = 'CHATGPTWEB_ACCESSTOKEN'
// AccessToken TTL 1小时
...
export class ChatGPTWeb {
  ...
  async sendMessage(
    ...
    if (!!opts.conversationId !== !!opts.parentMessageId) {
      throw new Error(
        'ChatGPTWeb.sendMessage: conversationId and parentMessageId must both be set or both be undefined'
      )
    }

    if (opts.conversationId && !isValidUUIDv4(opts.conversationId)) {
      throw new Error(
        'ChatGPTWeb.sendMessage: conversationId is not a valid v4 UUID'
      )
    }

    if (opts.parentMessageId && !isValidUUIDv4(opts.parentMessageId)) {
      throw new Error(
        'ChatGPTWeb.sendMessage: parentMessageId is not a valid v4 UUID'
      )
    }

    if (opts.messageId && !isValidUUIDv4(opts.messageId)) {
      throw new Error(
        'ChatGPTWeb.sendMessage: messageId is not a valid v4 UUID'
      )
    }
