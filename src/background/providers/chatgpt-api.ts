...
export class ChatGPTAPI {
  ...
  protected _systemMessage: string | undefined
  ...
  protected async _buildMessages(text: string, opts: types.SendMessageOptions) {
    ...
    do {
      ...
      // 限定上下文的条数
      if (messages.length > MAX_CONTEXT_COUNT) {
        break
      }
  ...
  protected async _defaultGetMessageById(
    id: string
  ): Promise<types.ChatMessage | undefined> {
  ...
  protected async _defaultUpsertMessage(
    ...
    // 会话上下文保持20分钟
    const { id, role, text, name, parentMessageId } = message;
    await this._messageStore.set(message.id, { id, role, text, name, parentMessageId }, 20 * 60 * 1000)
  }
}